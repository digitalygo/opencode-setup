...
- use token from `~/Documents/.secrets/mistral-key`
...
- Type: `string`
- Format: `file`
- A `.md` file is written to the same directory as the input file, using the same base filename with `.md` suffix. For example, `docs/inbox/example.pdf` produces `docs/inbox/example.md`.
...
**Base64 document URL OCR** (inlined in the request body; used because Mistral upload rejects these formats):

`.doc`, `.ppt`, `.rtf`, `.html`

**Key caveat**: Mistral's file upload endpoint is stricter than its OCR processing engine. The base64 fallback suffixes above are rejected by the upload endpoint but succeed when the file is sent as a base64-encoded data URI in the `document_url` field. This skill tries upload first for every supported suffix; when upload fails for a base64 fallback suffix, it automatically retries via the base64 path.
...
if input_path.is_symlink():
    raise PermissionError(f"Symlinks are not allowed: {input_path}")
...
secrets_path = Path.home() / "Documents" / ".secrets" / "mistral-key"
...
with Mistral(api_key=api_key) as client:
    ...
    try:
        ...
        if ext not in BASE64_FALLBACK_SUFFIXES:
            raise

        mime_type = MIME_FOR_BASE64.get(ext, "application/octet-stream")
